...
def test_make_fid():
    ...
    # --- 2D ---
    pts, sw, offset = 2048, 2000.0, 1000.0
    ...
    assert np.argmax(abs_spectrum_2d(fid)) == pts * int(0.375 * pts) + int(0.625 * pts)
    ...
    assert np.argmax(abs_spectrum_2d(fid)) == pts * int(0.75 * pts) + int(0.25 * pts)
...
def test_get_timepoints():
    ...
    for pt, st in zip(pts, sts):
        assert np.array_equal(
            sig.get_timepoints(expinfo, start_time=st)[0], test + (pt / sw)
        )

    # non-zero start times, using floats
    for st in ([0.03], [-0.04]):
        assert np.array_equal(sig.get_timepoints(expinfo, start_time=st)[0], test + st)
    ...
    assert all(
        same(a, b)
        for a, b in zip(sig.get_timepoints(expinfo, start_time=start_time), test2)
    )
    ...
    assert all(
        same(a, b)
        for a, b in zip(sig.get_timepoints(expinfo, start_time=start_time), test3)
    )
...
def test_random_signal():
    # --- 1D ---
    pts, sw, offset = 4048, 2000.0, 1000.0
    expinfo = ExpInfo(pts=pts, sw=sw, offset=offset)
    ...
    # --- 2D ---
    pts, sw, offset = 128, 2000.0, 1000.0
    expinfo = ExpInfo(pts=pts, sw=sw, offset=offset, dim=2)
    fid, tp, params = sig._generate_random_signal(10, expinfo)
