...
class TestSyntheticEstimator:
    ...
    expinfo_2d = ExpInfo(pts=512, sw=5000, offset=2000, sfo=500, nuclei="1H", dim=2)
    ...
    expinfo_2d_no_sfo = ExpInfo(pts=512, sw=5000, offset=2000, nuclei="1H", dim=2)
    ...
    def estimator_1d(self, sfo=True, nuclei=True):
        ...
        return Estimator.new_synthetic_from_parameters(
            self.params_1d, expinfo, snr=40.0
        )
    ...
    def estimator_2d(self, sfo=True, nuclei=True):
        ...
        return Estimator.new_synthetic_from_parameters(
            self.params_2d, expinfo, snr=40.0
        )
    ...
    def test_init(self):
        ...
        assert str(exc_info.value) == f"{RED}`params` should be a numpy array.{END}"
        ...
        with pytest.raises(ValueError) as exc_info:
            Estimator.new_synthetic_from_parameters(self.params_1d, self.expinfo_2d)
    ...
    def test_sw(self):
        for i in range(1, 3):
            ...
            assert estimator.get_sw() == tuple([5000.0 for _ in range(i)])
            assert estimator.get_sw(unit="ppm") == tuple([10.0 for _ in range(i)])

            with pytest.raises(_errors.InvalidUnitError) as exc_info:
                estimator.get_sw(unit="blah")
            assert str(exc_info.value) == (
                f"{RED}unit should be one of the following: 'hz', " f"'ppm'{END}"
            )
    ...
    def test_offset(self):
        for i in range(1, 3):
            estimator = getattr(self, f"estimator_{i}d")()
            assert estimator.get_offset() == tuple([2000.0 for _ in range(i)])
            assert estimator.get_offset(unit="ppm") == tuple([4.0 for _ in range(i)])
    ...
    def test_sfo(self):
        for i in range(1, 3):
            estimator = getattr(self, f"estimator_{i}d")()
            assert estimator.sfo == tuple([500.0 for _ in range(i)])
            estimator = getattr(self, f"estimator_{i}d")(sfo=False)
    ...
    def frequency_filter(self):
        ...
        estimator.frequency_filter(
            region=((3250.0, 2900.0),),
            noise_region=((1000.0, 1500.0),),
            region_unit="hz",
        )
...
def test_synthetic_estimator():
    ...
    # --- Time-points ------------------------------------------------
    tp = np.linspace(0.0, (n[0] - 1) / sw[0], n[0])

    assert np.array_equal(
        np.round(estimator.get_timepoints()[0], decimals=4), np.round(tp, decimals=4)
    )
    ...
    region_hz = [[3350.0, 2800.0]]
    noise_region_hz = [[2000.0, 1500.0]]
    region_ppm = [[3350.0 / sfo[0], 2800.0 / sfo[0]]]
    noise_region_ppm = [[2000.0 / sfo[0], 1500.0 / sfo[0]]]
    ...
    # Hz
    estimator.frequency_filter(region_hz, noise_region_hz, region_unit="hz")
...
def test_bruker_estimator():
    ...
    # --- Time-points ------------------------------------------------
    tp = np.round(
        np.linspace(0.0, (pts - 1) / sw_h, pts),
        decimals=4,
    )
    ...
    fid = sig.make_fid(params, n, sw, offset=offset, snr=30.0)[0]
    estimator.data = fid
    estimator.sw = sw
    ...
    for method, word in zip(
        (estimator.plot_result, estimator.write_result), ("plotting", "saving")
    ):
        with pytest.raises(ValueError) as exc_info:
            method()
    ...
    estimator.result = np.array([[1.0, 0.0, -4.0, 3.0], [2.0, 1.0, 2.0, 3.0]])
    estimator.add_oscillators(np.array([[1.0, 0.0, 0.0, 1.0]]))
    assert np.array_equal(
        estimator.result,
        np.array(
            [
    ...
    assert np.array_equal(estimator.result, np.array([[3.0, 0.5, 1.0, 2.0]]))
