...
class MainSetup(tk.Frame):
    def __init__(self, master, fidpath, pdatapath):
        ...
        self.buttonframe = ButtonFrame(self.rightframe, self.pad, bounds_ppm,
                                       ...
                                       self.off_p)
        self.contactrame = ContactFrame(self.rightframe, self.pad)
...
class ContactFrame(tk.Frame):
    def __init__(self, master, pad):
        ...
        email = tk.Label(self, text='simon.hulse@chem.ox.ac.uk', font='Courier',
                         bg='white')
        feedback.grid(row=0, column=0, sticky='w', padx=self.pad, pady=(self.pad, 0))
        email.grid(row=1, column=0, sticky='w', padx=self.pad, pady=(0, self.pad))
