...
def load_bruker(directory, ask_convdta=True):
    ...
    **For Development**
    ...
    if ask_convdta:
        convdta_prompt = (
            f'{cols.O}WARNING: It is necessary that the FID you import has been'
            ' digitally filtered, using the <convdta> command in TopSpin. If'
            ' you have not done this, please do so before proceeding\nIf you'
            ' wish to proceed, enter [y]\nIf you wish to quit, enter [n]:'
            f'{cols.END} '
        )
    ...
    # --- Search for data and parameter files ----------------------------
    ...
    if info['dim'] == 2:
        p, file = ('TD', 'acqu2s') if info['dtype'] == 'fid' else ('SI', 'proc2s')
