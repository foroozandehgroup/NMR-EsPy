...
def _write_txt(info, table, path, descrip, timestamp):
    """
    _write_txt(table, date, time, fname, dir, descrip, datapath, sw, offset,
              sfo, region, dim)

    Description
    -----------
    Writes result of NMR-EsPy to a textfile with location specified by
    |path|.
    ...
    table - numpy.ndarray
        ...
        by _constr_datatable()
    timestamp - str
        Time\date when write_file() is called. Generated by _timestamp()
    path - str
        Path to textfile.
    descrip - str or None
    ...
    datapath - str or None
        The path to the data analysed, if it exists. If None, this is
        skipped.
    sw - tuple
    ...
    sfo - tuple
        Transmitter frequency (MHz) in each dimension.
    region - list or None
        The region analysed, if it was specified, in both Hz and ppm.
        If None, this is skipped.
        Structure:
        region[0] - Hz values
        region[1] - ppm values
        region[0][0], region[0][1], ... - Hz values in dim 1, dim 2, ...
        region[1][0], region[1][1], ... - ppm values in dim 1, dim 2, ...
        region[0][0][0], region[0][0][1] - L and R Hz values in dim 1
    dim - int
...
def _write_pdf(info, table, path, descrip, timestamp):
    """
    write_pdf(table, timestamp, path, descrip, datapath, sw, off, sfo,
              region, sf, dim)

    Description
    -----------
    Writes result of NMR-EsPy to a pdf with location specified by
    |path|. Requires a LaTeX installation to compile the .tex file
    ...
    region - tuple or None
        The region analysed, if it wsa specified. If None, this is
        skipped.
    dim - int
        Signal dimension. Should be 1 or 2.
    """
...
def _constr_datatable(res, sfo, integrals, sf, sci_lims, dim):
    """
    _constr_datatable(res, sfo, integrals, sf, dim)

    Description
    -----------
    Creates an array of values to input to results file
    Contents per oscillator are:
    number, amplitude, phase, freq(s) in Hz and ppm, damp(s),
    integral (absolute and normalised)
...
def _get_integrals(res, sw_h, off_h, n, dim):
    """_get_integrals(para, sw, off, n, dim)

    Description
    -----------
    Determine the integrals of peaks corressponding to each
    oscillators in a parameter array. Uses simpson's rule
    to determine integrals. See |scipy.integrate.simps|
    ...
    integs - numpy.ndarray
        Integral for each peak in the results, of shape (M,)
    """
