...
# Last Edited: Sun 04 Jun 2023 23:45:19 BST
...
class BBQChili(ne.Estimator1D):
    ...
    def quadratic_phase(self) -> np.ndarray:
        ...
        phi1 = shifts * (1.e-6 * self.prescan_delay + self.pulse_length / 2)
        phi2 = (shifts ** 2) * (self.pulse_length / (2 * self.pulse_bandwidth))
        return self.spectrum * np.exp(-2j * np.pi * (phi1 + phi2))
    ...
    def back_extrapolate(self) -> np.ndarray:
        ...
        for oscillator in self.get_params():
            ...
            fid += a * np.exp(1j * p) * np.exp((2j * np.pi * (f - self.offset()[0]) - d) * timepoints)
    ...
    def _time_offset(self, freq: float) -> float:
        ...
        """
        return (
            (1.e-6 * self.prescan_delay) +
            (self.pulse_length / 2) -
            ((freq - self.offset()[0]) * self.pulse_length / self.pulse_bandwidth)
        )
